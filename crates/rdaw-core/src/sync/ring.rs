...
pub fn buffer_with_userdata<T, U>(
    ...
    let buffer = ManuallyDrop::new(ProcessLocalBuffer::new(capacity, userdata));
...
pub struct ProcessLocalBuffer<T, U = ()> {
    inner: NonNull<Header<U>>,
    marker: PhantomData<T>,
}

unsafe impl<T: Send + Sync, U: Send + Sync> Send for ProcessLocalBuffer<T, U> {}
unsafe impl<T: Send + Sync, U: Send + Sync> Sync for ProcessLocalBuffer<T, U> {}

impl<T, U> ProcessLocalBuffer<T, U> {
    fn new(capacity: usize, userdata: U) -> ProcessLocalBuffer<T, U> {
...
unsafe impl<T, U> Buffer<T, U> for ProcessLocalBuffer<T, U> {
    fn userdata(&self) -> &U {
...
impl<T, U> Drop for ProcessLocalBuffer<T, U> {
    fn drop(&mut self) {
...
pub struct Producer<T, U = (), B: Buffer<T, U> = ProcessLocalBuffer<T, U>> {
...
pub struct Consumer<T, U = (), B: Buffer<T, U> = ProcessLocalBuffer<T, U>> {
    buffer: ManuallyDrop<B>,
    closed: bool,
    read_idx: usize,
    write_state: usize,
