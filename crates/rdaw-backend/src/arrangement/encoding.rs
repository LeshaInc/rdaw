...
pub fn serialize(ctx: &SerializationContext<'_>, arrangement: &Arrangement) -> Result<Vec<u8>> {
    let tempo_map_uuid = ctx.get_uuid(arrangement.tempo_map_id)?;
    let main_track_uuid = ctx.get_uuid(arrangement.main_track_id)?;
...
pub fn deserialize(ctx: &DeserializationContext<'_>, data: &[u8]) -> Result<Arrangement> {
    ...
    let tempo_map_id = ctx.get_id(raw.tempo_map_uuid)?;
    let main_track_id = ctx.get_id(raw.main_track_uuid)?;
